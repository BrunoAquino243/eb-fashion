...
function addToCart(productId) {
    try {
        ...
        if (!productId) {
            ...
            alert('Erro ao adicionar produto ao carrinho. ID inválido.');
            return;
        }
        ...
        if (!product) {
            ...
            console.log("Produtos disponíveis:", getProducts());
...
function addItemToCart(product, quantity, size = null) {
    try {
        ...
        // Obter a imagem correta para o carrinho
        ...
        if (existingItemIndex !== -1) {
            ...
            cart.push({
                ...
                name: product.name,
                price: product.price,
                image: productImage, // Usar a imagem processada
                quantity: quantity,
                size: size
            });
        }
...
function loadCartItems() {
    ...
    cart.forEach((item, index) => {
        // Usar a imagem já armazenada no item do carrinho
