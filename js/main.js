...
function getProductImage(product) {
    ...
    if (product.image.includes('upload_')) {
        const imageKey = product.image.split('/').pop(); // Pega apenas o nome do arquivo
    ...
    // Se não for uma imagem carregada pelo usuário ou não encontrada no localStorage, usar o caminho normal
...
function loadCartItems() {
    ...
    cart.forEach((item, index) => {
        // Obter a imagem correta (local ou base64)
        const product = { image: item.image };
        const imagePath = getProductImage(product);
